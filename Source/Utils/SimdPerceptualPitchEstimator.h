...
public:
    ...
    /**
     * @brief Calculates the perceptual pitch offset for calibration.
     * 
     * @param frequency Current F0 in Hz.
     * @param rms Current RMS energy in dB (e.g., -12.0).
     * @return float Pitch offset in cents.
     */
    static float getPerceptualOffset(float frequency, float rms) {
        if (frequency < 1000.0f) {
            // Low frequency compensation for high energy
            return (rms > -12.0f) ? 2.0f : 0.0f; // +2 cents
        } else if (frequency > 2500.0f) {
            // High frequency compensation for high energy
            return (rms > -12.0f) ? -3.0f : 0.0f; // -3 cents
        }
        return 0.0f;
    }
