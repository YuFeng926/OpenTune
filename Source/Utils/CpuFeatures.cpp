...
int CpuFeatures::getRecommendedThreadCount() const {
    // 对于音频处理，使用物理核心数通常比逻辑核心数效果更好
    // 因为超线程在浮点密集型任务上帮助有限
    int recommended = physicalCores_;
    
    // 至少 2 个，最多 8 个（避免过度并行化）
    recommended = std::max(2, std::min(8, recommended));
