    ...
    : AudioProcessorEditor(&p), processorRef_(p), menuBar_(p), topBar_(menuBar_, transportBar_), arrangementView_(p)
{
    ...
    transportBar_.onFileMenuRequested = [this]() {
        juce::PopupMenu menu = menuBar_.getMenuForIndex(0, "File");
    ...
    transportBar_.onEditMenuRequested = [this]() {
        juce::PopupMenu menu = menuBar_.getMenuForIndex(1, "Edit");
    ...
    transportBar_.onViewMenuRequested = [this]() {
        juce::PopupMenu menu = menuBar_.getMenuForIndex(2, "View");
    ...
    // Initialize RenderingManager progress reporting (Assuming we can access it from here)
    // Note: RenderingManager is in Processor, we poll it in timer.
...
bool OpenTuneAudioProcessorEditor::keyPressed(const juce::KeyPress& key)
{
    ...
    // This catches key presses that bubble up from child components (if not consumed)
...
{
    auto* renderingManager = processorRef_.getRenderingManager();
    const bool inferenceNow = pianoRoll_.isAutoTuneProcessing()
        || (renderingManager != nullptr && renderingManager->isRendering());
    ...
    if (allowSecondaryRefresh && !f0ParamsSyncedFromInference_ && processorRef_.isInferenceReady()) {
        ...
        parameterPanel_.setF0Min(inference.getF0Min());
        parameterPanel_.setF0Max(inference.getF0Max());
    ...
    if (renderingManager != nullptr) {
        ...
        const auto& txn = processorRef_.getCurrentTxn();
        const int plannedChunks = txn.planned.load();
        const int convergedChunks = txn.converged.load();
        const int errorChunks = txn.errors.load();
        const bool isTxnActive = (plannedChunks > (convergedChunks + errorChunks));
        ...
        // AUTO overlay 释放判定：事务终态收敛
        if (autoOverlayLatched_) {
            bool shouldUnlatch = false;
            if (!autoOverlayTargetExists) {
                shouldUnlatch = true;
            } else {
                const bool txnMatches = (txn.trackId == autoOverlayTargetTrackId_ 
                    && txn.clipId == autoOverlayTargetClipId_);
                const bool txnFinished = (plannedChunks == (convergedChunks + errorChunks) && plannedChunks > 0);
                
                if (!isAutoProcessing && txnMatches && txnFinished) {
                    shouldUnlatch = true;
                    if (errorChunks == 0) {
                        AppLogger::log("AUTO_OVERLAY_SUCCESS id=" + juce::String(static_cast<juce::int64>(txn.id))
                            + " converged=" + juce::String(convergedChunks)
                            + "/" + juce::String(plannedChunks));
                    } else {
                        AppLogger::log("AUTO_OVERLAY_PARTIAL id=" + juce::String(static_cast<juce::int64>(txn.id))
                            + " converged=" + juce::String(convergedChunks)
                            + "/" + juce::String(plannedChunks)
                            + " errors=" + juce::String(errorChunks));
                    }
                }
            }
...
{
    pianoRoll_.setActiveTrackId(trackId);
    ...
    const auto* clipBuffer = processorRef_.getClipAudioBuffer(trackId, clipIndex);
    const bool hasUserAudio = (clipBuffer != nullptr);
...
{
    // 参数拖动结束时记录 Undo Action
    using PT = ParameterChangeAction::ParamType;
    PT type;
    ...
    switch (paramId) {
        case 0: // RetuneSpeed
            type = PT::RetuneSpeed;
            applier = [this](float v) {
                parameterPanel_.setRetuneSpeed(v);
                retuneSpeedChanged(v);
            };
            break;
        case 1: // VibratoDepth
            type = PT::VibratoDepth;
            applier = [this](float v) {
                parameterPanel_.setVibratoDepth(v);
                vibratoDepthChanged(v);
            };
            break;
        case 2: // VibratoRate
            type = PT::VibratoRate;
            applier = [this](float v) {
                parameterPanel_.setVibratoRate(v);
                vibratoRateChanged(v);
            };
            break;
        case 3: // NoteSplit
            type = PT::NoteSplit;
            applier = [this](float v) {
                parameterPanel_.setNoteSplit(v);
                noteSplitChanged(v);
            };
            break;
        default:
    ...
    processorRef_.getUndoManager().addAction(
        std::make_unique<ParameterChangeAction>(type, oldValue, newValue, std::move(applier)));
}
        ...
        [safeThis, trackId, fileName = file.getFileName()](AsyncAudioLoader::LoadResult result)
        {
                                              ...
                                              audioBuffer = std::move(result.audioBuffer)]() mutable
            {
                ...
                juce::MessageManager::callAsync([safeThis, trackId, prepared = std::move(prepared)]() mutable
                {
                    ...
                    safeThis->pianoRoll_.setActiveTrackId(trackId);
                    ...
                    const auto* clipBuffer = safeThis->processorRef_.getClipAudioBuffer(trackId, clipIndex);
...
void OpenTuneAudioProcessorEditor::performScaleInferenceForClip(int trackId, int clipIndex)
{
    ...
    if (curve)
    {
        ...
        const float voicedRatio = f0.empty() ? 0.0f : (static_cast<float>(voicedFrames) / static_cast<float>(f0.size()));
...
void OpenTuneAudioProcessorEditor::requestOriginalF0ExtractionForImport(int trackId, int clipIndex)
{
    ...
    auto submitResult = f0ExtractionService_.submit(
        ...
        [safeThis, trackId, clipIndex, clipId, requestKey]() -> F0ExtractionService::Result {
            ...
            auto extractionGuard = InferenceManager::getInstance().beginOriginalF0Extraction();
            ...
            if (clipId != 0) {
                ...
                const auto* clipBuffer = procRef.getClipAudioBuffer(trackId, clipIndex);
                if (clipBuffer == nullptr || clipBuffer->getNumSamples() <= 0) {
                    out.errorMessage = "empty_clip_audio";
                    return out;
                }
                snap.audioBuffer.makeCopyOf(*clipBuffer);
            }
            ...
            // Use internal sample rate (44.1kHz) for all calculations
            constexpr double internalSampleRate = SilentGapDetector::kInternalSampleRate;

            auto& inference = InferenceManager::getInstance();
            const int hopSize = inference.getF0HopSize();
            const int f0SampleRate = inference.getF0SampleRate();

            std::vector<SilentGap> silentGaps = SilentGapDetector::detectAllGapsAdaptive(snap.audioBuffer);
            ...
            for (const auto& seg : voicedSegments) {
                ...
                const auto result = inference.extractF0WithEnergy(segAudio.data(), segLength, static_cast<int>(internalSampleRate), nullptr, nullptr);
                ...
                for (size_t i = 0; i < result.first.size() && segFrameOffset + i < totalFrames; ++i) {
                    fullF0[segFrameOffset + i] = result.first[i];
                    if (i < result.second.size()) {
                        fullEnergy[segFrameOffset + i] = result.second[i];
                    }
                }
            }
...
void OpenTuneAudioProcessorEditor::pitchCurveEdited(int startFrame, int endFrame)
{
    ...
    if (hopSize <= 0 || f0SampleRate <= 0.0) {
        auto& inference = InferenceManager::getInstance();
        hopSize = inference.getF0HopSize();
        f0SampleRate = (double) inference.getF0SampleRate();
    }
    if (hopSize <= 0 || f0SampleRate <= 0.0) {
        return;
    }
    ...
    const auto* clipBuffer = processorRef_.getClipAudioBuffer(trackId, clipIndex);
    const double clipDurationSec = clipBuffer 
        ? static_cast<double>(clipBuffer->getNumSamples()) / SilentGapDetector::kInternalSampleRate 
        : 0.0;
    ...
    const auto& gaps = processorRef_.getClipSilentGaps(trackId, clipIndex);
    ...
    auto findChunkAlignedRange = [&](double startSec, double endSec) -> std::pair<double, double> {
        if (chunkBoundaries.size() < 2) {
            return { std::max(0.0, startSec), std::min(clipDurationSec, endSec) };
        }
        
        double chunkStart = -1.0;
        double chunkEnd = -1.0;
        
        for (size_t i = 0; i + 1 < chunkBoundaries.size(); ++i) {
            const double cStart = chunkBoundaries[i];
            const double cEnd = chunkBoundaries[i + 1];
            
            if (startSec < cEnd && endSec > cStart) {
                if (chunkStart < 0.0 || cStart < chunkStart) {
                    chunkStart = cStart;
                }
                if (chunkEnd < 0.0 || cEnd > chunkEnd) {
                    chunkEnd = cEnd;
                }
            }
        }
        
        if (chunkStart >= 0.0 && chunkEnd > chunkStart) {
    ...
    if (!curve->hasRenderableCorrectedF0()) {
        const uint64_t clipId = processorRef_.getClipId(trackId, clipIndex);
        const auto chunkRange = findChunkAlignedRange(editStartSec, editEndSec);
        processorRef_.clearClipCacheRange(trackId, clipIndex, chunkRange.first, chunkRange.second);
        AppLogger::log("RenderTrace: pitchCurveEdited early-return noRenderableCorrectedF0"
    ...
    {
        bool anyVisibleInEditedRange = curve->hasCorrectionInRange(startFrame, endFrame + 1);

        if (!anyVisibleInEditedRange) {
            anyVisibleInEditedRange = curve->hasCorrectedVisibleInRange(editStartSec, editEndSec);
    ...
    double renderStartSec = editStartSec;
    double renderEndSec = editEndSec;

    bool hasLineAnchorInEditedRange = false;
    bool hasNonLineAnchorInEditedRange = false;
    {
        auto snapshot = curve->getSnapshot();
        const auto& segs = snapshot->getCorrectedSegments();
        for (const auto& seg : segs) {
            if (seg.endFrame <= startFrame || seg.startFrame >= (endFrame + 1)) {
                continue;
            }

            if (seg.source == CorrectedSegment::Source::LineAnchor) {
                hasLineAnchorInEditedRange = true;
            } else {
                hasNonLineAnchorInEditedRange = true;
            }

            if (hasLineAnchorInEditedRange && hasNonLineAnchorInEditedRange) {
                break;
            }
        }
    }

    const bool useStrictDirtyRangeForRender = hasLineAnchorInEditedRange && !hasNonLineAnchorInEditedRange;

    if (!useStrictDirtyRangeForRender) {
        const auto chunkRange = findChunkAlignedRange(renderStartSec, renderEndSec);
        renderStartSec = chunkRange.first;
        renderEndSec = chunkRange.second;
    }
    ...
    AppLogger::log("RenderTrace: pitchCurveEdited"
        ...
        + " secRange=[" + juce::String(renderStartSec, 3) + "," + juce::String(renderEndSec, 3) + "]");

    processorRef_.enqueuePartialRender(trackId, clipIndex, renderStartSec, renderEndSec);
    pianoRoll_.setRenderingProgress(1.0f, 0);
}
...
void OpenTuneAudioProcessorEditor::notesChanged(const std::vector<Note>& notes)
{
    int trackId = processorRef_.getActiveTrackId();
    int clipIndex = processorRef_.getSelectedClip(trackId);
