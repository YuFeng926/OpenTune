...
// ============================================================================
// PianoRollRenderer - 钢琴卷帘渲染器
// ============================================================================

void PianoRollRenderer::prepareWaveformThumbCache(const juce::AudioBuffer<float>* buffer, int sampleRate)
{
    if (!buffer || buffer->getNumSamples() == 0)
    {
        AppLogger::debug("[PianoRollRenderer] prepareWaveformThumbCache: buffer is null or empty, clearing cache");
        waveformThumbCache_ = WaveformThumbCache{};
        return;
    }

    const int numSamples = buffer->getNumSamples();

    if (waveformThumbCache_.buffer != buffer || waveformThumbCache_.numSamples != numSamples)
    {
        AppLogger::debug("[PianoRollRenderer] prepareWaveformThumbCache: new buffer detected, rebuilding cache "
            "(numSamples=" + juce::String(numSamples) + ", sampleRate=" + juce::String(sampleRate) + ")");
        waveformThumbCache_.buffer = buffer;
        waveformThumbCache_.numSamples = numSamples;
        waveformThumbCache_.sampleRate = sampleRate;

        waveformThumbCache_.numThumbSamples = (numSamples + WaveformThumbCache::samplesPerThumbSample - 1)
                                              / WaveformThumbCache::samplesPerThumbSample;

        AppLogger::debug("[PianoRollRenderer] prepareWaveformThumbCache: computed "
            + juce::String(waveformThumbCache_.numThumbSamples) + " thumb samples needed");

        waveformThumbCache_.thumbData.assign(static_cast<std::size_t>(waveformThumbCache_.numThumbSamples), MinMaxValue());
        waveformThumbCache_.numThumbSamplesFinished = 0;
        waveformThumbCache_.thumbComplete = false;
    }
}

bool PianoRollRenderer::buildWaveformThumbSlice(double timeBudgetMs)
{
    if (waveformThumbCache_.thumbComplete) {
        AppLogger::debug("[PianoRollRenderer] buildWaveformThumbSlice: already complete, returning false");
        return false;
    }
    if (waveformThumbCache_.buffer == nullptr) {
        AppLogger::warn("[PianoRollRenderer] buildWaveformThumbSlice: buffer is null");
        return false;
    }
    if (waveformThumbCache_.numSamples <= 0) {
        AppLogger::warn("[PianoRollRenderer] buildWaveformThumbSlice: numSamples <= 0");
        return false;
    }
    if (waveformThumbCache_.numThumbSamples <= 0 || waveformThumbCache_.thumbData.size() != static_cast<std::size_t>(waveformThumbCache_.numThumbSamples)) {
        AppLogger::warn("[PianoRollRenderer] buildWaveformThumbSlice: invalid thumbData state");
        return false;
    }

    AppLogger::debug("[PianoRollRenderer] buildWaveformThumbSlice: starting with timeBudgetMs=" + juce::String(timeBudgetMs)
        + ", remaining=" + juce::String(waveformThumbCache_.numThumbSamples - waveformThumbCache_.numThumbSamplesFinished)
        + " of " + juce::String(waveformThumbCache_.numThumbSamples) + " thumb samples");

    const double startMs = juce::Time::getMillisecondCounterHiRes();
    bool progressed = false;

    const auto* audioData = waveformThumbCache_.buffer->getReadPointer(0);
    const int numSamples = waveformThumbCache_.numSamples;

    while (waveformThumbCache_.numThumbSamplesFinished < waveformThumbCache_.numThumbSamples)
    {
        const int batchSize = 256;
        const int numToDo = juce::jmin(batchSize,
                                       waveformThumbCache_.numThumbSamples - waveformThumbCache_.numThumbSamplesFinished);

        if (numToDo <= 0)
        {
            waveformThumbCache_.thumbComplete = true;
            break;
        }

        const int startThumb = waveformThumbCache_.numThumbSamplesFinished;

        for (int i = 0; i < numToDo; ++i)
        {
            const int thumbIdx = startThumb + i;
            const int sampleStart = thumbIdx * WaveformThumbCache::samplesPerThumbSample;
            const int sampleEnd = juce::jmin(sampleStart + WaveformThumbCache::samplesPerThumbSample,
                                             numSamples);
            const int numSamplesBatch = sampleEnd - sampleStart;

            if (numSamplesBatch > 0)
            {
                const auto range = juce::FloatVectorOperations::findMinAndMax(
                    audioData + sampleStart, numSamplesBatch);
                waveformThumbCache_.thumbData[static_cast<std::size_t>(thumbIdx)].setFloat(range);
            }
        }

        waveformThumbCache_.numThumbSamplesFinished += numToDo;
        progressed = true;
    ...
    if (waveformThumbCache_.numThumbSamplesFinished >= waveformThumbCache_.numThumbSamples)
    {
        waveformThumbCache_.thumbComplete = true;
        AppLogger::debug("[PianoRollRenderer] buildWaveformThumbSlice: COMPLETE! "
            + juce::String(waveformThumbCache_.numThumbSamples) + " thumb samples generated");
    }
...
void PianoRollRenderer::drawWaveform(juce::Graphics& g, const RenderContext& ctx)
{
    ...
    if (!ctx.hasUserAudio || waveformThumbCache_.numThumbSamplesFinished <= 0)
        return;

    AppLogger::debug("[PianoRollRenderer] drawWaveform: rendering "
        + juce::String(waveformThumbCache_.numThumbSamplesFinished) + " thumb samples");
    ...
    const double sr = ctx.audioSampleRate;
    ...
    const double timePerPixel = clipVisibleDuration / static_cast<double>(w);

    g.setColour(UIColors::waveformFill.withAlpha(0.2f));

    juce::Path waveformPath;
    const double timeToThumbFactor = sr / WaveformThumbCache::samplesPerThumbSample;

    for (int x = startX; x < endX; ++x)
    {
        const double clipTime = startClipTime + (x - startX) * timePerPixel;
        const int rawThumbStart = juce::roundToInt(clipTime * timeToThumbFactor);
        const int rawThumbEnd = juce::roundToInt((clipTime + timePerPixel) * timeToThumbFactor);

        const int thumbStart = juce::jmax(0, rawThumbStart);
        if (thumbStart >= waveformThumbCache_.numThumbSamples)
            continue;

        int thumbEnd = juce::jmax(thumbStart + 1, rawThumbEnd);
        thumbEnd = juce::jmin(thumbEnd, waveformThumbCache_.numThumbSamples);

        juce::Range<float> aggregated(0.0f, 0.0f);
        bool hasData = false;

        for (int t = thumbStart; t < thumbEnd; ++t)
        {
            if (t < waveformThumbCache_.numThumbSamplesFinished)
            {
                const auto r = waveformThumbCache_.thumbData[static_cast<std::size_t>(t)].getRange();
                if (waveformThumbCache_.thumbData[static_cast<std::size_t>(t)].isNonZero())
                {
                    if (!hasData)
                    {
                        aggregated = r;
                        ...
                        aggregated = aggregated.getUnionWith(r);
                    }
                }
            }
        }
        ...
        if (hasData)
        {
            const float yMin = centerY - aggregated.getEnd() * amplitudeScale;
            const float yMax = centerY - aggregated.getStart() * amplitudeScale;
