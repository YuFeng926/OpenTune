...
private:
    ...
    struct WaveformCache
    {
        ...
        int numChannels{ 0 };
        int width{ 0 };                    // 目标像素宽度
        std::vector<float> peaks;          // 每像素一个峰值
        std::size_t nextX{ 0 };            // 增量构建进度
        bool complete{ false };
    };

    uint64_t getClipCacheKey(int trackId, int clipIndex) const;
    WaveformCache& getOrCreateWaveformCache(uint64_t key);
    static void prepareWaveformCache(WaveformCache& cache, const juce::AudioBuffer<float>& audio, int width);
    static bool buildWaveformCacheSlice(WaveformCache& cache, double timeBudgetMs);
    static bool pruneWaveformCaches(std::unordered_map<uint64_t, WaveformCache>& caches, const std::unordered_set<uint64_t>& alive);
    bool buildWaveformCaches(double timeBudgetMs);

    std::unordered_map<uint64_t, WaveformCache> waveformCaches_;
